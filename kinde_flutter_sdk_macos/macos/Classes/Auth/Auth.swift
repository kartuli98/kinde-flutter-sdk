...
//Explanation:
//NSApplication.shared.windows: Retrieves all the windows in the macOS app.
//isKeyWindow: Identifies the window that is currently active (receiving keyboard events).
//contentViewController: The macOS equivalent of rootViewController. It represents the primary view controller associated with the window.
//presentedViewControllers: In macOS, multiple view controllers can be presented, so we check the presentedViewControllers array.
  private func getViewController() async -> NSViewController? {
      await MainActor.run {
          // Get the key window (the one currently receiving user input)
          let keyWindow = NSApplication.shared.windows.first { $0.isKeyWindow }
          
          // Get the content view controller of the key window (similar to rootViewController in iOS)
          var topController = keyWindow?.contentViewController
          
          // Traverse through presented view controllers if any
          while let presentedViewController = topController?.presentedViewControllers?.first {
    ...
    private func getAuthorizationRequest(signUp: Bool,
        ...
        return try await withCheckedThrowingContinuation { continuation in
            Task {
                let issuerUrl = config.getDomainUrl()
    ...
    private func runCurrentAuthorizationFlow(request: OIDAuthorizationRequest, viewController: NSViewController) async throws -> Bool {
        return try await withCheckedThrowingContinuation { continuation in
            Task {
                await MainActor.run {
                    currentAuthorizationFlow = OIDAuthState.authState(byPresenting: request,
                                                                      presenting: viewController.view.window!,
  ...
  private func discoverConfiguration(issuerUrl: URL,
      ...
      return try await withCheckedThrowingContinuation { continuation in
          OIDAuthorizationService.discoverConfiguration(forIssuer: issuerUrl) { configuration, error in
              // Early exit for error cases, ensuring continuation is only resumed once
              if let error = error {
              ...
              // Continue with the success path only after ensuring there are no errors
              do {
                  guard let redirectUrl = self.config.getRedirectUrl() else {
                  ...
                  // Creating the request only once all parameters are valid
                  ...
                  // Resume continuation only after creating the request successfully
                  continuation.resume(returning: request)
                  
              } catch {
                  // Catch any unexpected errors and pass them to the continuation
                  continuation.resume(throwing: error)
              }
          }
      }
